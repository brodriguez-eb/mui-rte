...
const styles = (theme: Theme & TMUIRichTextEditorStyles) =>
  createStyles({
    ...
    //   fontSize: "1rem",
    //   translate: "1% 160%",
    //   transition: "color 200ms cubic-bezier(0, 0, 0.2, 1) 0ms, translate 200ms cubic-bezier(0, 0, 0.2, 1) 0ms, font 200ms cubic-bezier(0, 0, 0.2, 1) 0ms;",
    // },
    ...
    placeHolderAnimationPreTransition: {
      position: "absolute",
      translate: "12px 16px",
      fontSize: "1rem",
      transition: "color 200ms cubic-bezier(0, 0, 0.2, 1) 0ms, translate 200ms cubic-bezier(0, 0, 0.2, 1) 0ms, font 200ms cubic-bezier(0, 0, 0.2, 1) 0ms;",
    },
    placeHolderAnimationTransition: {
      fontSize: "0.765rem",
      translate: "12px 7px",
      color: theme.palette.primary.main,
    },
...
const useEditorState = (props: IMUIRichTextEditorProps) => {
  ...
  return defaultValue ? EditorState.createWithContent(convertFromRaw(JSON.parse(defaultValue)), decorator) : EditorState.createEmpty(decorator);
};
...
const MUIRichTextEditor: ForwardRefRenderFunction<TMUIRichTextEditorRef, IMUIRichTextEditorProps> = (props, ref) => {
  ...
  const getAutocompleteItems = (): TAutocompleteItem[] => {
    ...
    return autocompleteRef.current!.items.filter((item) => item.keys.filter((key) => key.includes(searchTerm)).length > 0).splice(0, autocompleteLimit);
  };
  ...
  const handlePlaceholderFocus = () => {
    if (isFirstFocus.current === false) return focusEditor();
  ...
  const handleInsertAtomicBlockAsync = (name: string, promise: Promise<TAsyncAtomicBlockResponse>, placeholder?: string) => {
    ...
    promise
      ...
      .catch((error) => {
        ...
        const newContentState = Modifier.removeRange(editorStateRef.current!.getCurrentContent(), newSelection as SelectionState, "forward");
  ...
  return (
    <div id={`${editorId}-root`} className={classes.root}>
      ...
      >
          ...
          >
            <label
              className={classNames(
                classes.placeHolderAnimationPreTransition,
                classes.placeHolder,
                { [classes.error]: props.error },
                (focus || hasText) && classes.placeHolderAnimationTransition
              )}
            >
              {props.label || ""}
            </label>

            <div id={`${editorId}-editor-container-inside`} className={classNames(classes.editorContainerInside)}>
              <Editor
        ...
        {/* <TextField id="outlined-multiline-static" label={props.label} multiline variant="filled" rows={8} sx={{ width: "100%" }} /> */}

        {state.anchorUrlPopover && (
          <UrlPopover data={state.urlData} anchor={state.anchorUrlPopover} onConfirm={handleConfirmPrompt} isMedia={state.urlIsMedia} />
        )}
      </div>
    </div>
  );
...
export default withStyles(styles, { withTheme: true, name: "MuiRichTextEditorBase" })(forwardRef(MUIRichTextEditor));
